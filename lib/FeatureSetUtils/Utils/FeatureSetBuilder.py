...
class FeatureSetBuilder:
    ...
    def _validate_upload_featureset_from_diff_expr_params(self, params):
        ...
        if fold_scale_type not in validate_fold_scale_type:
            raise ValueError('Input fold scale type value [{}] is not valid'.format(
                                                                            fold_scale_type))

    def _generate_report(self, feature_set_ref, feature_ids, genome_id, workspace_name):
        """
        ...
        output_html_files = self._generate_html_report(feature_ids, genome_id)
        ...
        report_params = {
              ...
              'objects_created': [{
                                'ref': feature_set_ref,
                                'description': 'FeatureSet object generated by FeatureSetUtils'}],
    ...
    def _generate_html_report(self, feature_ids, genome_id):
        """
        ...
        for feature_id in feature_ids:
            feature_ids_content += '<tr><td>{}</td><td>'.format(feature_id)
    ...
    def _process_diff_expression(self, diff_expression_ref, result_directory):
        ...
        return:
        diff_expr_matrix_file: the differential expression matrix file generated by upstream apps
        genome_id: genome object ref associated with differential expression object
        """

        log('start processing differential expression object')

        diff_expression_data = self.ws.get_objects2(
                                    {'objects': [{'ref': diff_expression_ref}]})['data'][0]['data']

        genome_id = diff_expression_data['genome_id']

        handle_id = diff_expression_data['file']['hid']
        ...
        if diff_expr_matrix_file_name not in result_files:
            raise ValueError('"gene_results.csv" is not availale in DifferentialExpression object')
    ...
    def _process_matrix_file(self, diff_expr_matrix_file, comp_p_value, comp_q_value,
        ...
        with open(diff_expr_matrix_file, 'r') as file:
            ...
            for row in reader:
                feature_id = row[''].split('.')[0]
                row_p_value = row['pvalue']
                row_q_value = row['padj']
    ...
    def upload_featureset_from_diff_expr(self, params):
        ...
        feature_ids = self._process_matrix_file(diff_expr_matrix_file,
                                                params.get('p_cutoff'),
                                                params.get('q_cutoff'),
                                                params.get('fold_scale_type'),
                                                params.get('fold_change_cutoff'))

        feature_set_ref = self._generate_feature_set(feature_ids,
                                                     genome_id,
                                                     params.get('workspace_name'),
                                                     params.get('feature_set_name'))

        returnVal = {'result_directory': result_directory,
                     'feature_set_ref': feature_set_ref}

        report_output = self._generate_report(feature_set_ref,
                                              feature_ids,
                                              genome_id,
                                              params.get('workspace_name'))
