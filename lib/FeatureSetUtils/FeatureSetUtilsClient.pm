...
=head2 filter_expression_matrix_with_feature_set

  $returnVal = $obj->filter_expression_matrix_with_feature_set($params)
...
<pre>
$params is a FeatureSetUtils.FilterExpressionMatrixWithFeatureSetParams
$returnVal is a FeatureSetUtils.FilterExpressionMatrixWithFeatureSetResult
...
=cut
 ...
 sub filter_expression_matrix_with_feature_set
{
    my($self, @args) = @_;
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function filter_expression_matrix_with_feature_set (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to filter_expression_matrix_with_feature_set:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "FeatureSetUtils.filter_expression_matrix_with_feature_set",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
=head2 FilterExpressionMatrixWithFeatureSetParams
...
=head2 FilterExpressionMatrixWithFeatureSetResult
